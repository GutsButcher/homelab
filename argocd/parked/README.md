...
| `rustfs.yaml` | Parked at the owner's request. The manifests are complete and it deployed cleanly, but `s3.gwynbliedd.com` / `s3-api.gwynbliedd.com` have no Cloudflare Tunnel ingress rule, so they returned 502. Enabling it re-creates the namespace, deployment, PVC, sealed secret and Traefik routes. |
